...
```{r, fig.cap="Post-dive duration vs. dive duration"}
...
```{r, fig.cap="Post-dive duration vs. dive duration (raw data)"}
# dive duration vs pdi by days
ggplot(data = data_2018_filter[pdi < 300,], aes(x = dduration, 
  ...
  theme_jjo()
```
```{r, fig.cap="Post-dive duration / dive duration ratio vs. day since departure"}
# dive duration vs pdi by days
ggplot(
...
```{r, fig.cap="Distribution of the number of dives each day. The threshold used to calculate bADL is fixed at 50 dives per day.", fig.height=3}
...
```{r, fig.cap="Behavioral ADL vs. drift rate along animals' trip (Am I the only one seeing some kind of relationship?)"}
