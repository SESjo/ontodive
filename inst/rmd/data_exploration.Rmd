...
```{r fig.cap="Check for missing values", fig.width=9}
...
```{r, fig.cap='Distribution of `dduration` for each seal. The red line highlight the "subjective" threshold used to remove outliers', fig.width=9}
...
```{r, fig.cap='Same distribution of `dduration` for each seal but after removing any `dduration` < 3000 sec. The red line highlight the "subjective" threshold used to remove outliers', fig.width=9}
...
```{r, results='asis', fig.width=9, cache=TRUE}
...
### All Variables during the first month {.tabset}
...
```{r, fig.cap="Correlation matrix (crosses indicate non significant correlation)", fig.width=12}
# compute correlation
...
```{r, fig.cap="Dive duration vs. Maximum Depth colored by Dive Type", fig.width=9}
# plot
...
```{r, fig.cap="Drift rate vs. Bottom time", fig.width=9}
# plot
...
```{r, fig.cap="Drift rate vs. Maximum depth", fig.width=9}
# plot
ggplot(data_2018_filter[,.(driftrate=median(driftrate,na.rm=T),
                           botttime=median(botttime,na.rm=T),
                           maxdepth=median(maxdepth,na.rm=T),
...
```{r, fig.cap="Drift rate vs. Dive duration", fig.width=9}
# plot
ggplot(data_2018_filter[,.(driftrate=median(driftrate,na.rm=T),
                           botttime=median(botttime,na.rm=T),
                           maxdepth=median(maxdepth,na.rm=T),
