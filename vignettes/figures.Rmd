...
max_depth_all <- max_depth_all_ini +
  ...
  theme(
    ...
    axis.line.y = element_line(color = "black", arrow = arrow(length = unit(0.2, "lines"), 
                                                              type = "closed",
                                                              ends = "first")),
  )
...
# # plot
# plot_to_save <-
  (dive_duration_all | fig_dens_dive_duration_all) + plot_layout(widths = c(5, 1))
# 
# # to save
...
data_comp %>%
  ...
  merge(., data_comp[, .(nb_days = as.numeric(difftime(max(date), 
                                                       ...
                                                       units = "day"))),
                     by = .(.id, sp_rename)] %>%
          .[, .(nb_days = paste(round(mean(nb_days, na.rm = T), 1),
                                "±",
                                round(sd(nb_days, na.rm = T),1))),
            by = .(sp_rename)], by = "sp_rename") %>% 
  gt() %>% 
  tab_spanner(
    label = md("Dive"),
