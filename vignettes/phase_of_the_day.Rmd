...
# Let's give a try to the `findTwilights` function {.tabset}

Let's first reshape our data to complied with the `findTwilights` function.

```{r, fig.cap="Visualization of light level at the surface along 2018-individuals' trip, with twilight detection points", fig.height=6}
...
```{r, fig.cap="Visualization of light level at the surface along 2018-individuals' trip, with twilight detection points corrected", fig.height=6}
# display the result
ggplot() +
  geom_tile(data = dataPlot, aes(x = day_departure, y = hour, fill = lightatsurf)) + 
...
## Add condition on the distribution of `period_time`
...
```{r, fig.cap="Distributions of the time difference between two rows identified as sunrise and sunset"}
# display
...
```{r, fig.cap="Same as above, but `cluster 1 = cluster 1`, `cluster 2 = all the others`", fig.height=6}
# display the result
ggplot() +
  geom_tile(data = dataPlot[!is.na(lightatsurf),
                            ][,cluster:=res_dbscan$cluster
